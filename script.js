...
function render() {
  ...
  grid.innerHTML = list.map(p => {
    ...
    // Foto fotorealista gerada sob demanda a partir do nome do produto.
    // Isso evita imagens genéricas/ilustradas e funciona diretamente no GitHub Pages.
    const prompt = `professional ecommerce product photo, photorealistic, clean white studio background, centered product, no text, no people, product: ${p.name}`;
    const image = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=700&height=700&model=flux&nologo=true&safe=true&seed=${encodeURIComponent(p.id)}`;
    return `<article class="card ${chosen ? "chosen" : ""}">
      <div class="product-image-wrap"><img class="product-image" src="${image}" alt="${escapeAttr(p.name)}" loading="lazy" onerror="this.onerror=null;this.src='https://picsum.photos/seed/${encodeURIComponent(p.id)}/700/700';"></div>
